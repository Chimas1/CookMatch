...
// Rezept-Grunddaten
$rezept = $db->preparedStm("SELECT * FROM Rezepte WHERE id = ?", "i", $id);
if ($row = $rezept->fetch_assoc()) {
    ...
    // Zutatenliste
    $zutaten = $db->preparedStm("SELECT * FROM Lebensmitteln WHERE rezept_id = ?", "i", $id);
    ...
    // Kochutensilien
    $utensilien = $db->preparedStm("SELECT * FROM Kochutensilien WHERE rezept_id = ?", "i", $id);
    ...
    // Anweisungen
    $anweisungen = $db->preparedStm("SELECT * FROM Anweisung WHERE rezept_id = ? ORDER BY schritt_nr ASC", "i", $id);
    ...
    // Bewertungen
    $bewertungen = $db->preparedStm("SELECT * FROM bewertet WHERE rezept_id = ?", "i", $id);
