...
if ($row = $rezept->fetch_assoc()) {
    ...
    // Zutatenliste
    $zutaten = $db->preparedStm("SELECT * FROM Zutaten WHERE rezept_id = ?", "i", $id);
    ...
    // Kochutensilien
    $utensilien = $db->preparedStm("SELECT * FROM Utensilien WHERE rezept_id = ?", "i", $id);
    ...
    // Anweisungen
    $anweisungen = $db->preparedStm("SELECT * FROM Anweisungen WHERE rezept_id = ? ORDER BY schritt_nr ASC", "i", $id);
    ...
    // Bewertungen
    $bewertungen = $db->preparedStm("SELECT * FROM Bewertungen WHERE rezept_id = ?", "i", $id);
