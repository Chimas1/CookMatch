...
if ($row = $rezept->fetch_assoc()) {
   ...
   echo "<div class='section-title'>Zutaten</div>
    ...
    // Kochutensilien
    $utensilien = $db->select("SELECT Kochutensilien.Titel distinct FROM Kochutensilien, Braucht, Anweisung, Rezept WHERE Kochutensilien.Titel=Braucht.Titel and Braucht.ID=Anweisung.ID and Anweisung.Name=Rezept.Name and Rezept.Name = ?", "s", $id);
